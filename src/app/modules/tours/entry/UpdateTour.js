import { Badge, Card, Center, Divider, FileInput, Flex, Image, Select, Text, TextInput, Textarea } from "@mantine/core"
...
export const UpdateTour = ({ dataSource, update }) => {
    ...
    const [packageId, setPackageId] = useState(dataSource ? dataSource?.package_id : '');
    const [title, setTitle] = useState(dataSource ? dataSource?.title : '');
    const [tourCode, setTourCode] = useState(dataSource ? dataSource?.tour_code : '');
    const [destination, setDestination] = useState(dataSource ? dataSource?.destination : '');
    const [duration, setDuration] = useState(dataSource ? dataSource?.duration : '');
    const [highlight, setHighlight] = useState(dataSource ? dataSource?.highlight : '');
    ...
    const submitUpdateCountry = async () => {
        ...
        const response = await putRequest(`tour/update/${dataSource?.id}`, mainPayload);
